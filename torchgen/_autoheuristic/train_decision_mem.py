...
class AHTrainDecisionTree(AHTrain):
    ...
    def main(
        ...
        # Add a model_id column to uniquely identify each decision tree config
        # Add model_id column to identify each config
        agg = (
            results_df[results_df["set"].isin(["test", "val"])]
            .groupby(["crit", "max_depth", "min_samples_leaf"])[["correct", "total", "unsure"]]
            .sum()
        )
        agg["accuracy"] = agg["correct"] / agg["total"]
        best_config = agg.sort_values("accuracy", ascending=False).iloc[0]
        best_crit, best_depth, best_leaf = agg.sort_values("accuracy", ascending=False).index[0]

        # Step 2: Filter original results_df for this config
        mask = (
            (results_df["crit"] == best_crit) &
            (results_df["max_depth"] == best_depth) &
            (results_df["min_samples_leaf"] == best_leaf)
        )

        best_rows = results_df[mask]

        # Step 3: Print accuracies
        # for subset in ["val", "test"]:
        #     row = best_rows[best_rows["set"] == subset].iloc[0]
        #     acc = row["correct"] / row["total"]
        #     print(f"{subset.capitalize()} accuracy: {int(row['correct'])}/{int(row['total'])} = {acc:.2%}")

        # Step 4: Print combined accuracy
        # print(f"Combined test+val accuracy: {int(best_config['correct'])}/{int(best_config['total'])} = {best_config['accuracy']:.2%}")
        print(ALPHA, BETA, int(best_config['correct']), int(best_config['unsure']), int(best_config['total']))
    ...
    def get_df(
        ...
        df["score"] = (ALPHA * df["feedback"]) + (BETA * df["energy_joules"])
