...
DATA_COL = "median_execution_time"

# FEEDBACK_COL = "sm_utilization_after"
# DATA_COL = "median_sm_utilization_after"

# FEEDBACK_COL = "memory_utilization_after"
# DATA_COL = "median_memory_utilization_after"
    ...
    def get_df(self, log_path, cat_feature2cats=None, nrows=None, apply_filters=False):
        ...
        def process_data(
            ...
            def get_winner_and_speedups(group):
                mean_time = group[DATA_COL].mean()
                winner = group.loc[group[DATA_COL].idxmin(), CHOICE_COL]
                # winner = group.loc[group[DATA_COL].idxmax(), CHOICE_COL]

                min_time = group[DATA_COL].min()
                max_time = group[DATA_COL].max()

                group["winner"] = winner
                # group["speedup"] = max_time / min_time
                # print(group[DATA_COL])
                # print(mean_time)
                # exit()
                group[DATA_COL] = group[DATA_COL].replace(0, mean_time)
                group["target"] = mean_time / group[DATA_COL]
                # group["target"] = group[DATA_COL]


                return group[
                    feature_columns + [CHOICE_COL, "winner", "target"] #"speedup", "target"]
                ]
