...
SHARED_MEM_PER_BLOCK = 48 * 1024       # 48 KB = 49152 bytes
SHARED_MEM_PER_THREAD = 64             # Conservative estimate per-thread usage

def estimate_shared_memory_usage(block_m: int, block_n: int, block_k: int, num_warps: int, num_stages: int) -> int:
    threads_per_block = num_warps * 32
    total_threads = threads_per_block * num_stages
    # Approximate memory needed for operand tiles
    return total_threads * SHARED_MEM_PER_THREAD + (block_m + block_n + block_k) * 2  # bytes
...
class BaseConfigHeuristic(metaclass=BaseHeuristicSingleton):
    ...
    def __init__(self) -> None:
        ...
        self.mm_configs: list[BaseConfig] = [
            ...
            GemmConfig(16, 32, 16, 1, 2),
            GemmConfig(32, 16, 16, 1, 2),
        ...
        # ]
        self.extra_mm_configs: list[BaseConfig] = [
            GemmConfig(16, 16, 32, 1, 2),
            GemmConfig(16, 32, 32, 2, 2),
            GemmConfig(32, 16, 32, 2, 2),
            GemmConfig(32, 32, 16, 2, 4),
            GemmConfig(32, 32, 32, 2, 8),  # aggressive but still safe
        ]
        ...
        # ]
        self.persistent_mm_configs: list[BaseConfig] = [
            GemmConfig(16, 16, 16, 1, 4),
            GemmConfig(16, 32, 16, 2, 4),
            GemmConfig(32, 16, 16, 1, 8),
            GemmConfig(32, 32, 16, 2, 8),
            GemmConfig(32, 32, 32, 2, 4),
        ]
